...
`
  ...
  let ctx = $state<TopographyCtx | null>(null)

  const CTRL_INDICES: readonly (readonly [number, number, number, number])[] = [
    [1, -2, 3, -4],
    [9, -8, 7, -6],
    [5, 2, 5, -5],
    [-1, -3, 8, 9],
...
<div
  class={`topography-container ${className}`.trim()}
  bind:this={container}
></div>
