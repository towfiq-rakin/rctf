...
const withInstancerChallenge = async (
  { res, db, redis, challengeId, includeHidden, user }: InstanceRequest,
  run: (
    challenge: Challenge,
  ...
  if (redis && config.maxInstances !== undefined) {
...
export const createInstance = async <
  Res extends InstancerResponseHelpers & {
    badTooManyInstances?: () => any
  },
>(
  request: Omit<InstanceRequest, 'res'> & { res: Res }
): Promise<
  InstanceResponse | ReturnType<NonNullable<Res['badTooManyInstances']>>
> => {
  const { challenge, provider, error } = await getInstancerChallenge(
    request.res,
    request.db,
  ...
  const timeoutMilliseconds =
    challenge.data.instancerConfig!.timeoutMilliseconds
  const reservation =
    request.redis && typeof request.res.badTooManyInstances === 'function'
      ? await requestInstanceReservation(
          request.redis,
          request.db,
          request.user.id,
          challenge.id,
  ...
  if (reservation === 'limit') {
    return request.res.badTooManyInstances!()
  }
  if (reservation === 'unavailable') {
    return request.res.badInstancerError({
      message: 'Could not verify the active instance limit',
    })
  }

  let instanceStatus: instanceDetailsOrError
  try {
    instanceStatus = await provider.createInstance(
      await buildCreateInstanceOptions(request.db, challenge, request.user)
    )
  } catch (err) {
    if (reservation === 'reserved' && request.redis) {
      await recordInstanceStopped(request.redis, request.user.id, challenge.id)
    }
    throw err
  }
  ...
  if (reservation !== 'disabled' && !isExistingTerminal && request.redis) {
